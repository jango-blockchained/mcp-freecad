...
def demo_connection(connection_method=None):
    ...
    # Create connection
    fc = FreeCADConnection(prefer_method=connection_method, auto_connect=True)
    ...
    try:
        ...
        export_result = fc.export_stl(
            object_name=None, file_path=output_file, document=doc_name
        )

        if not export_result:
            raise Exception("Export failed (method returned False)")

        is_mock = fc.get_connection_type() == fc.CONNECTION_MOCK
        if is_mock:
            ...
            if os.path.exists(output_file) and os.path.getsize(output_file) > 0:
                print(
                    f"✅ Verified file exists: {output_file} ({os.path.getsize(output_file)} bytes)"
                )
            else:
                print(
                    f"⚠️ File not found or empty: {output_file}. Export might require an object name."
                )
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--method",
        choices=["auto", "server", "bridge", "mock"],
        default="auto",
        help="Connection method to test",
    )
    parser.add_argument(
        "--test-all", action="store_true", help="Test all connection methods"
    )
    parser.add_argument(
        "--start-server",
        action="store_true",
