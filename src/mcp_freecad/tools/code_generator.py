...
import Part
"""
            if "import FreeCAD" not in script_content and "import App" not in script_content:
                script_content = imports + script_content
            
            # Add document creation if not present
            if "App.newDocument" not in script_content and "FreeCAD.newDocument" not in script_content:
        ...
        try:
                    ...
                    for file_path in self.snippets_dir.glob("*.py"):
                        try:
                            ...
                            snippet = {
                                ...
                                "created_at": metadata.get("created", datetime.datetime.now().isoformat()),
                            ...
                            logger.error(f"Error loading snippet from file {file_path}: {e}")
            ...
            if snippet_id and snippet_id in self._snippet_library:
                ...
                for s in self._snippet_library.values():
                    ...
                    return {"status": "error", "message": f"Snippet with name '{name}' not found"}
            else:
                return {"status": "error", "message": f"Snippet with ID '{snippet_id}' not found"}
    ...
    async def _get_script_history(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "message": f"Error retrieving script history: {str(e)}"}
    
    async def _generate_primitive_script(self, params: Dict[str, Any]) -> Dict[str, Any]:
...
"""
                ...
                return {"status": "error", "message": f"Unsupported primitive type: {primitive_type}"}
...
"""
            ...
            return {"status": "error", "message": f"Error generating primitive script: {str(e)}"}
    
    async def _generate_transform_script(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not transform_type:
                return {"status": "error", "message": "No transformation type specified"}
...
"""
                ...
                return {"status": "error", "message": f"Unsupported transformation type: {transform_type}"}
...
"""
            ...
            return {"status": "error", "message": f"Error generating transform script: {str(e)}"}
    ...
    async def _generate_boolean_script(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not operation_type:
                return {"status": "error", "message": "No boolean operation type specified"}
            
            # Get object names
            object1 = params.get("object1", "")
            object2 = params.get("object2", "")
            if not object1 or not object2:
                return {"status": "error", "message": "Two object names are required for boolean operations"}
...
"""
                ...
                return {"status": "error", "message": f"Unsupported boolean operation type: {operation_type}"}
...
"""
            ...
            return {"status": "error", "message": f"Error generating boolean operation script: {str(e)}"}
