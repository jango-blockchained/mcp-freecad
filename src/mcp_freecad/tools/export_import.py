...
class ExportImportToolProvider(ToolProvider):
    ...
    EXPORT_FORMATS = {
        "step": {"extension": ".step", "description": "STEP (ISO 10303-21) - Standard for Exchange of Product Data"},
        "stp": {"extension": ".stp", "description": "STP (ISO 10303-21) - Alternative extension for STEP"},
        "iges": {"extension": ".iges", "description": "IGES - Initial Graphics Exchange Specification"},
        "igs": {"extension": ".igs", "description": "IGS - Alternative extension for IGES"},
        "brep": {"extension": ".brep", "description": "BREP - Boundary representation file format"},
        "stl": {"extension": ".stl", "description": "STL - Stereolithography format for 3D printing"},
        "obj": {"extension": ".obj", "description": "OBJ - Wavefront 3D object file"},
        "dxf": {"extension": ".dxf", "description": "DXF - AutoCAD Drawing Exchange Format"},
        "wrl": {"extension": ".wrl", "description": "VRML - Virtual Reality Modeling Language"},
        "vrml": {"extension": ".vrml", "description": "VRML - Alternative extension for VRML"},
        "fcstd": {"extension": ".FCStd", "description": "FCStd - Native FreeCAD format"}
    }
    ...
    async def _export_file(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if format_name in ["step", "stp"]:
                ...
                mesh = Mesh.Mesh()
                for shape in shapes:
                    mesh_data = Mesh.Mesh(shape.tessellate(params.get("tessellation", 1.0)))
                ...
                try:
                    ...
                    return {"status": "error", "message": f"Export module not found: {exporter_name}"}
    ...
    async def _import_file(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if format_name in ["step", "stp"]:
                ...
                doc = self.app.getDocument(os.path.splitext(os.path.basename(file_path))[0])
