...
class ModelManipulationToolProvider(ToolProvider):
    ...
    async def _transform_object(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not object_name:
                return {"status": "error", "message": "No object specified for transformation"}
            ...
            if not hasattr(obj, "Placement"):
                return {"status": "error", "message": f"Object {object_name} does not support transformations"}
            ...
            if translation:
                if isinstance(translation, list) and len(translation) == 3:
                    ...
                    vec = self.app.Vector(translation[0], translation[1], translation[2])
                    
                    # Create a new placement with the translation
                    new_placement = self.app.Placement(
                        current_placement.Base.add(vec),
                    ...
                    modifications.append(f"Translated by ({translation[0]}, {translation[1]}, {translation[2]})")
                else:
                    return {"status": "error", "message": "Invalid translation format. Expected [x, y, z]"}
                ...
                if isinstance(rotation, list) and len(rotation) >= 3:
                    ...
                    if len(rotation) == 3:
                        ...
                        new_rotation = current_placement.Rotation.multiply(
                            self.app.Rotation(
                                self.app.Vector(1, 0, 0), rot_x
                            ).multiply(
                                self.app.Rotation(
                                    self.app.Vector(0, 1, 0), rot_y
                                )
                            ).multiply(
                                self.app.Rotation(
                                    self.app.Vector(0, 0, 1), rot_z
                                )
                            )
                        )
                        ...
                        modifications.append(f"Rotated by ({rot_x}°, {rot_y}°, {rot_z}°)")
                    elif len(rotation) == 4:
                        ...
                        # Create rotation
                        new_rotation = current_placement.Rotation.multiply(
                            self.app.Rotation(self.app.Vector(axis_x, axis_y, axis_z), angle)
                        )
            ...
            return {"status": "error", "message": f"Error transforming object: {str(e)}"}
    ...
    async def _boolean_operation(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not object1 or not object2:
                return {"status": "error", "message": "Two objects are required for boolean operations"}
            ...
            if not hasattr(obj1, "Shape") or not hasattr(obj2, "Shape"):
                return {"status": "error", "message": "Objects must have shapes for boolean operations"}
            
            # Perform the boolean operation
            if operation_type.lower() == "union":
                result_shape = obj1.Shape.fuse(obj2.Shape)
                operation_name = "Union"
            elif operation_type.lower() == "difference" or operation_type.lower() == "cut":
                result_shape = obj1.Shape.cut(obj2.Shape)
                operation_name = "Difference"
            elif operation_type.lower() == "intersection" or operation_type.lower() == "common":
            ...
            return {"status": "error", "message": f"Error performing boolean operation: {str(e)}"}
    ...
    async def _fillet_edge(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            object_name = params.get("object")
            if not object_name:
                return {"status": "error", "message": "No object specified for filleting"}
            ...
            if edge_indices:
                ...
                for idx in edge_indices:
                    if idx < len(obj.Shape.Edges):
                        ...
                        return {"status": "error", "message": f"Edge index out of range: {idx}"}
            else:
                # Use all edges
    ...
    async def _chamfer_edge(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            object_name = params.get("object")
            if not object_name:
                return {"status": "error", "message": "No object specified for chamfering"}
            
            distance = params.get("distance")
            if distance is None:
                return {"status": "error", "message": "No distance specified for chamfer"}
    ...
    async def _mirror_object(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            object_name = params.get("object")
            if not object_name:
                return {"status": "error", "message": "No object specified for mirroring"}
            ...
            if not plane and not normal:
                return {"status": "error", "message": "No mirror plane or normal vector specified"}
            ...
            if plane:
                ...
                if plane.lower() == "xy" or plane.lower() == "z":
                    ...
                    return {"status": "error", "message": f"Unknown plane: {plane}. Use 'xy', 'xz', 'yz', 'x', 'y', or 'z'"}
            else:
                # Custom normal vector
                if isinstance(normal, list) and len(normal) == 3:
                    mirror_vec = self.app.Vector(normal[0], normal[1], normal[2])
                else:
                    return {"status": "error", "message": "Invalid normal vector format. Expected [x, y, z]"}
            ...
            # Return the result
            plane_name = plane if plane else f"normal vector ({normal[0]}, {normal[1]}, {normal[2]})"
            return {
                "status": "success",
    ...
    async def _scale_object(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if scale is None and scale_x is None and scale_y is None and scale_z is None:
    ...
    async def _offset_object(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result_shape = obj.Shape.makeOffset(offset, tolerance, intersection, self_intersection, join_type)
    ...
    async def _thicken_object(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            object_name = params.get("object")
            if not object_name:
                return {"status": "error", "message": "No object specified for thickening"}
