...
class ConstraintResourceProvider(ResourceProvider):
    ...
    async def get_resource(self, uri: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if len(path_parts) < 1 or path_parts[0] != "constraints":
            raise ValueError(f"Invalid URI format: {uri}, expected 'cad://constraints/...'")
        ...
        try:
            ...
            for obj in doc.Objects:
                if obj.TypeId == "Sketcher::SketchObject":
                    ...
                    constraints_count = len(obj.Constraints) if hasattr(obj, "Constraints") else 0
    ...
    def _mock_all_constraints(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sketch_constraints": {
                "count": 2,
                "sketches": [
                    {"sketch_name": "Sketch", "label": "Sketch", "constraints_count": 12},
                    {"sketch_name": "Sketch001", "label": "Sketch001", "constraints_count": 8}
                ]
            },
            "assembly_constraints": {
                "count": 3,
                "constraints": [
                    {"name": "Constraint001", "label": "FixedConstraint", "type": "Assembly2Constraint"},
                    {"name": "Constraint002", "label": "PlaneConstraint", "type": "Assembly2Constraint"},
                    {"name": "Constraint003", "label": "AxisConstraint", "type": "Assembly2Constraint"}
                ]
            },
            ...
            if hasattr(sketch, "Constraints"):
                for i, constraint in enumerate(sketch.Constraints):
                    ...
                    if constraint.Type == "Distance":
                        ...
                        constraint_data["second_pos"] = getattr(constraint, "Second", -1)
            ...
            if hasattr(sketch, "Geometry"):
                for i, geo in enumerate(sketch.Geometry):
                    ...
                    if geo_type == "LineSegment":
                        geo_data["start"] = [geo.StartPoint.x, geo.StartPoint.y, geo.StartPoint.z]
                        geo_data["end"] = [geo.EndPoint.x, geo.EndPoint.y, geo.EndPoint.z]
    ...
    def _mock_sketch_constraints(self, sketch_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "constraints": {
                "count": 6,
                "items": [
                    {"index": 0, "name": "Horizontal1", "type": "Horizontal", "driven": False, "pos": 0},
                    {"index": 1, "name": "Vertical1", "type": "Vertical", "driven": False, "pos": 1},
                    {"index": 2, "name": "Distance1", "type": "Distance", "driven": False, "value": 10.0, "first_pos": 0, "second_pos": -1},
                    {"index": 3, "name": "Distance2", "type": "Distance", "driven": False, "value": 20.0, "first_pos": 1, "second_pos": -1},
                    {"index": 4, "name": "Coincident1", "type": "Coincident", "driven": False, "first_pos": 0, "second_pos": 1},
                    {"index": 5, "name": "Parallel1", "type": "Parallel", "driven": False, "first_pos": 2, "second_pos": 3}
                ]
            },
            "geometry": {
                "count": 4,
                "items": [
                    {"index": 0, "type": "LineSegment", "construction": False, "start": [0.0, 0.0, 0.0], "end": [10.0, 0.0, 0.0]},
                    {"index": 1, "type": "LineSegment", "construction": False, "start": [10.0, 0.0, 0.0], "end": [10.0, 20.0, 0.0]},
                    {"index": 2, "type": "LineSegment", "construction": False, "start": [10.0, 20.0, 0.0], "end": [0.0, 20.0, 0.0]},
                    {"index": 3, "type": "LineSegment", "construction": False, "start": [0.0, 20.0, 0.0], "end": [0.0, 0.0, 0.0]}
                ]
            },
            "note": "Mock data (FreeCAD not available)"
        }
                ...
                if hasattr(obj, "Type") and "Constraint" in obj.Type:
                    if any(asm_type in obj.Type for asm_type in ["Assembly", "A2p"]):
                        ...
                        for prop in obj.PropertiesList:
                            if prop not in ["Name", "Label", "Type", "Object1", "Object2", "References"]:
                                try:
        ...
        return {
            ...
            "constraints": [
                ...
                },
                {
                    ...
                    "parameters": {"Offset": 0.0, "Plane1": "XY_Plane", "Plane2": "XY_Plane"}
                },
                {
                    "name": "Constraint003", 
    ...
    async def _get_object_assembly_constraints(self, object_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for doc_obj in doc.Objects:
                if hasattr(doc_obj, "Type") and "Constraint" in doc_obj.Type:
                    if any(asm_type in doc_obj.Type for asm_type in ["Assembly", "A2p"]):
                        is_related = False
