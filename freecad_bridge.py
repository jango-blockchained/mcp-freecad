...
"""
    ...
    def create_box(self, length: float = 10.0, width: float = 10.0, height: float = 10.0, 
        ...
        """
        doc_creation = f'doc = FreeCAD.newDocument("BoxDocument")' if doc_name is None else f'doc = FreeCAD.getDocument("{doc_name}")'
...
"""
        ...
        try:
            ...
            raise RuntimeError(f"Failed to create box: {stderr}")
        except Exception as e:
            raise RuntimeError(f"Error creating box: {e}")
    
    def export_stl(self, object_name: str, output_path: str, doc_name: Optional[str] = None) -> bool:
        """
        ...
        Returns:
            True if successful
        """
        doc_ref = 'doc = FreeCAD.ActiveDocument' if doc_name is None else f'doc = FreeCAD.getDocument("{doc_name}")'
