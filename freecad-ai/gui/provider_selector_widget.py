...
class ProviderSelectorWidget(QtWidgets.QWidget):
    ...
    def _refresh_providers(self):
        ...
        try:
            ...
            if active_count > 0:
                self._update_status_indicator("connected", f"{active_count}/{total_count} providers active")
            else:
    ...
    def _get_provider_models(self, provider_name, provider_info):
        ...
        try:
            ...
            if self.provider_service and hasattr(self.provider_service, 'get_provider_models'):
    ...
    def _on_model_selection_changed(self, model_name):
        ...
        if self.provider_service and self.current_provider:
            try:
                if hasattr(self.provider_service, 'update_provider_model'):
                    success = self.provider_service.update_provider_model(self.current_provider, model_name)
                    if success:
                        print(f"ProviderSelector: Updated model {model_name} for provider {self.current_provider}")
                    else:
                        print(f"ProviderSelector: Failed to update model via provider service")
                else:
                    print(f"ProviderSelector: Provider service doesn't support model updates")
            except Exception as e:
                print(f"ProviderSelector: Error updating model via provider service: {e}")
        ...
        if self.config_manager and self.current_provider:
            try:
                ...
                print(f"ProviderSelector: Saved model {model_name} for provider {self.current_provider}")
            except Exception as e:
    ...
    def is_provider_available(self):
        ...
        return (self.current_provider is not None and 
                self.current_provider in self.available_providers and
                self.available_providers[self.current_provider]['status'] == 'active')
