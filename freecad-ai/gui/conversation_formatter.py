...
class ConversationFormatter:
    ...
    def _format_markdown(self, sender: str, message: str, timestamp: str) -> str:
        ...
        # Code blocks
        formatted_message = re.sub(r'```(.*?)```', r'<pre>\1</pre>', formatted_message, flags=re.DOTALL)
        ...
        # Lists
        formatted_message = re.sub(r'^- (.*?)$', r'• \1', formatted_message, flags=re.MULTILINE)
        formatted_message = re.sub(r'^\* (.*?)$', r'• \1', formatted_message, flags=re.MULTILINE)
        formatted_message = re.sub(r'^\d+\. (.*?)$', r'⦿ \1', formatted_message, flags=re.MULTILINE)
        
        # Line breaks
        formatted_message = formatted_message.replace('\n', '<br>')
        
        if timestamp:
            header = f'<span style="color: #888; font-size: 10px;">[{timestamp}]</span> '
        else:
            header = ''
    ...
    def _process_rich_text_formatting(self, text: str) -> str:
        ...
        # Headers
        text = re.sub(r'^# (.*?)$', r'<h1 style="color: #333; margin: 10px 0 5px 0;">\1</h1>', text, flags=re.MULTILINE)
        text = re.sub(r'^## (.*?)$', r'<h2 style="color: #444; margin: 8px 0 4px 0;">\1</h2>', text, flags=re.MULTILINE)
        text = re.sub(r'^### (.*?)$', r'<h3 style="color: #555; margin: 6px 0 3px 0;">\1</h3>', text, flags=re.MULTILINE)
        
        # Lists with better styling
        text = re.sub(r'^- (.*?)$', r'<span style="color: #2196F3;">•</span> \1', text, flags=re.MULTILINE)
        text = re.sub(r'^\* (.*?)$', r'<span style="color: #2196F3;">•</span> \1', text, flags=re.MULTILINE)
        text = re.sub(r'^(\d+)\. (.*?)$', r'<span style="color: #2196F3;">\1.</span> \2', text, flags=re.MULTILINE)
        
        # Links (basic support)
        text = re.sub(r'\[([^\]]+)\]\(([^\)]+)\)', r'<a href="\2" style="color: #2196F3; text-decoration: underline;">\1</a>', text)
    ...
    def export_conversation(self, messages: list, format_type: ConversationFormat = None) -> str:
        ...
        for msg in messages:
            ...
            if self.format_type == ConversationFormat.HTML:
                css_class = "user" if sender.lower() == "you" else "ai" if sender.lower() in ["ai", "assistant"] else "system"
