...
def crash_safe_wrapper(operation_name):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                FreeCAD.Console.PrintMessage(f"FreeCAD AI: Starting {operation_name}...\n")
                result = func(*args, **kwargs)
                FreeCAD.Console.PrintMessage(f"FreeCAD AI: {operation_name} completed successfully\n")
                return result
...
def safe_signal_connect(signal, slot, operation_name="signal connection"):
    ...
    try:
        if signal and slot and hasattr(signal, 'connect'):
            ...
            FreeCAD.Console.PrintWarning(f"FreeCAD AI: {operation_name} skipped - invalid signal or slot\n")
            return False
    except Exception as e:
        FreeCAD.Console.PrintError(f"FreeCAD AI: CRASH PREVENTED in {operation_name}: {e}\n")
        FreeCAD.Console.PrintError(f"FreeCAD AI: {operation_name} traceback: {traceback.format_exc()}\n")
...
class MCPMainWidget(QtWidgets.QDockWidget):
    ...
    def __init__(self, parent=None):
        ...
        try:
            FreeCAD.Console.PrintMessage("FreeCAD AI: Starting ultra-safe widget initialization...\n")
    ...
    def _create_ultra_minimal_ui(self):
        ...
        try:
            ...
            FreeCAD.Console.PrintError(f"FreeCAD AI: Ultra-minimal UI creation failed: {e}\n")
            self.status_label = None
    ...
    def _initialize_full_functionality(self):
        ...
        if self._fully_initialized:
            return
        try:
            FreeCAD.Console.PrintMessage("FreeCAD AI: Starting simplified full initialization...\n")
            ...
            FreeCAD.Console.PrintMessage("FreeCAD AI: Simplified initialization completed successfully\n")
        except Exception as e:
            FreeCAD.Console.PrintError(f"FreeCAD AI: Simplified initialization failed: {e}\n")
            if hasattr(self, 'status_label') and self.status_label:
                try:
    ...
    def _init_services_safe(self):
        ...
        try:
            FreeCAD.Console.PrintMessage("FreeCAD AI: Initializing services safely...\n")
            ...
            FreeCAD.Console.PrintWarning(f"FreeCAD AI: Service initialization failed: {e}\n")
        ...
        try:
            ...
            for attr_name, class_name, tab_name in widget_configs:
                try:
                    ...
                    FreeCAD.Console.PrintMessage(f"FreeCAD AI: Successfully created {tab_name} tab\n")
                except Exception as e:
                    FreeCAD.Console.PrintWarning(f"FreeCAD AI: Failed to create {tab_name} tab: {e}\n")
                    placeholder = QtWidgets.QWidget()
                    placeholder_layout = QtWidgets.QVBoxLayout(placeholder)
                    error_label = QtWidgets.QLabel(f"{tab_name}\n\nLoading failed:\n{str(e)[:100]}...")
        ...
        try:
            ...
            if self.provider_service:
                if hasattr(self, 'providers_widget') and hasattr(self.providers_widget, 'set_provider_service'):
                    safe_widget_operation(
                        lambda: self.providers_widget.set_provider_service(self.provider_service),
                        "provider service connection to providers widget"
                    )
                if hasattr(self, 'conversation_widget') and hasattr(self.conversation_widget, 'set_provider_service'):
                    safe_widget_operation(
            ...
            if self.agent_manager:
                if hasattr(self, 'conversation_widget') and hasattr(self.conversation_widget, 'set_agent_manager'):
                    safe_widget_operation(
                        lambda: self.conversation_widget.set_agent_manager(self.agent_manager),
                        "agent manager connection to conversation widget"
                    )
                if hasattr(self, 'agent_control_widget') and hasattr(self.agent_control_widget, 'set_agent_manager'):
                    safe_widget_operation(
                        lambda: self.agent_control_widget.set_agent_manager(self.agent_manager),
                        "agent manager connection to agent control widget"
                    )
            FreeCAD.Console.PrintMessage("FreeCAD AI: Service connections complete\n")
        except Exception as e:
            FreeCAD.Console.PrintWarning(f"FreeCAD AI: Service connection failed: {e}\n")
    ...
    def _init_agent_manager_safe(self):
        ...
        try:
            ...
            safe_widget_operation(
                lambda: self.agent_manager.register_callback("on_mode_change", self._on_agent_mode_changed),
                "agent mode change callback registration"
            )
            safe_widget_operation(
                lambda: self.agent_manager.register_callback("on_state_change", self._on_agent_state_changed),
    ...
    def _on_provider_status_changed(
        ...
        if not hasattr(self, "status_label") or self.status_label is None:
            FreeCAD.Console.PrintMessage(f"FreeCAD AI: Status change ignored - no status_label: {provider_name} -> {status}\n")
            return
        try:
            if status == "connected":
            ...
            FreeCAD.Console.PrintError(f"FreeCAD AI: Error updating status label: {e}\n")

    @crash_safe_wrapper("providers update handling")
    def _on_providers_updated(self):
        """Handle providers list update."""
        if not hasattr(self, "status_label") or self.status_label is None:
            FreeCAD.Console.PrintMessage("FreeCAD AI: Providers update ignored - no status_label\n")
            return
        try:
            if self.provider_service:
            ...
            FreeCAD.Console.PrintError(f"FreeCAD AI: Error updating providers status: {e}\n")
    ...
    def _on_agent_mode_changed(self, old_mode, new_mode):
        ...
        if not hasattr(self, "status_label") or self.status_label is None:
            FreeCAD.Console.PrintMessage(f"FreeCAD AI: Agent mode change ignored - no status_label: {old_mode} -> {new_mode}\n")
            return
        try:
            mode_display = "Chat" if new_mode.value == "chat" else "Agent"
            self.status_label.setText(f"Mode: {mode_display}")
        except Exception as e:
            FreeCAD.Console.PrintError(f"FreeCAD AI: Error updating agent mode status: {e}\n")
    ...
    def _on_agent_state_changed(self, old_state, new_state):
        ...
        if not hasattr(self, "status_label") or self.status_label is None:
            FreeCAD.Console.PrintMessage(f"FreeCAD AI: Agent state change ignored - no status_label: {old_state} -> {new_state}\n")
            return
        try:
            state_display = {
                "idle": "Ready",
            ...
            FreeCAD.Console.PrintError(f"FreeCAD AI: Error updating agent state status: {e}\n")
