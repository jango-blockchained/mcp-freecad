...
class AgentDiagnosticDialog(QtWidgets.QDialog):
    ...
    def _update_agent_status(self):
        ...
        if self.agent_manager is None:
            ...
            try:
                ...
                if config:
                    status_lines.append(
                        f"   │  ├─ Require Approval: {config.get('require_approval', 'N/A')}"
                    )
                    status_lines.append(
                        f"   │  ├─ Auto Rollback: {config.get('auto_rollback', 'N/A')}"
                    )
                    status_lines.append(
                        f"   │  ├─ Safety Checks: {config.get('safety_checks', 'N/A')}"
                    )
                    status_lines.append(
                        f"   │  ├─ Execution Timeout: {config.get('execution_timeout', 'N/A')}s"
                    )
                    status_lines.append(
                        f"   │  └─ Max Retries: {config.get('max_retries', 'N/A')}"
                    )
                ...
                if hasattr(self.agent_manager, "tool_registry"):
                    ...
                    total_tools = sum(
                        len(methods) for methods in available_tools.values()
                    )
                ...
                if hasattr(self.agent_manager, "get_execution_history"):
                    ...
                    status_lines.append(
                        f"   └─ Execution History: {len(history)} entries"
                    )
    ...
    def _update_provider_status(self):
        ...
        if self.provider_service is None:
            ...
            try:
                ...
                current_selection = (
                    self.provider_service.get_current_provider_selection()
                )
                status_lines.append(
                    f"   ├─ Current Provider: {current_selection.get('provider', 'None')}"
                )
                status_lines.append(
                    f"   └─ Current Model: {current_selection.get('model', 'None')}"
                )
    ...
    def _update_tools_status(self):
        ...
        if self.agent_manager is None:
            ...
            try:
                available_tools = self.agent_manager.get_available_tools()
                status_lines.append(
                    f"✅ Tools Registry: {len(available_tools)} categories available"
                )

                for category, tools in available_tools.items():
                    status_lines.append(
                        f"   ├─ {category.title()} ({len(tools)} tools)"
                    )
    ...
    def _update_system_info(self):
        ...
        try:
            ...
            status_lines.append(
                f"FreeCAD Version: {'.'.join(str(v) for v in FreeCAD.Version()[:3])}"
            )
            status_lines.append(
                f"FreeCAD Build: {FreeCAD.Version()[3] if len(FreeCAD.Version()) > 3 else 'Unknown'}"
            )
        except Exception as e:
...
class EnhancedAgentControlWidget(QtWidgets.QWidget):
    ...
    def _create_enhanced_queue_section(self, layout):
        ...
        self.queue_list.setHeaderLabels(
            ["Task", "Priority", "Steps", "Est. Time", "Risk"]
        )
    ...
    def _show_advanced_diagnostics(self):
        ...
        dialog = AgentDiagnosticDialog(self.agent_manager, self.provider_service, self)
        dialog.exec_()
    ...
    def _show_quick_status(self):
        ...
        if self.provider_service:
            ...
            status_lines.append(
                f"Providers: {len(active_providers)}/{len(total_providers)} active"
            )
        else:
        ...
        QtWidgets.QMessageBox.information(self, "Quick Status", "\n".join(status_lines))
    ...
    def _step_execution(self):
        ...
        if first_item:
            ...
            complete_btn.clicked.connect(
                lambda: self._complete_step(dialog, first_item)
            )
    ...
    def _edit_queue_item(self):
        ...
        current_text = (
            ...
            else str(current_item.text())
        )
        current_priority = "Medium"  # Default
        current_steps = (
            current_item.text(2)
            if hasattr(current_item, "text") and current_item.columnCount() > 2
            else "1"
        )
        current_time = (
            current_item.text(3)
            if hasattr(current_item, "text") and current_item.columnCount() > 3
            else "5 min"
        )
        ...
        steps_spin.setValue(
            int(current_steps.split()[0]) if current_steps.split()[0].isdigit() else 1
        )
    ...
    def _export_settings(self):
        ...
        try:
            ...
            settings = {
                ...
                "ui_settings": {"queue_count": self.queue_list.count()},
            }
                ...
                if item:
                    item_data = {
                        ...
                        "background_color": (
                            ...
                            if item.background() != QtCore.Qt.NoBrush
                            else None
                        ),
                    }
