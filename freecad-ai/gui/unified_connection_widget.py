...
class UnifiedConnectionWidget(QtWidgets.QWidget):
    ...
    def _create_connection_cards(self, layout):
        ...
        self.server_start_btn = server_card.add_action_button("Start", "▶", self._start_server)
        self.server_stop_btn = server_card.add_action_button("Stop", "■", self._stop_server)
        self.server_restart_btn = server_card.add_action_button("Restart", "⟳", self._restart_server)
        ...
        claude_card.add_action_button("Test", "🔍", lambda: self._test_connection("claude"))
    ...
    def _test_connection(self, connection_type):
        ...
        QtCore.QTimer.singleShot(1000, lambda: self._add_activity(
    ...
    def _update_quick_status(self):
        ...
        active_count = sum(
            1 for card in self.connection_cards.values()
        ...
        if active_count == total_count:
            self.quick_status.setText(f"✅ All systems connected ({active_count}/{total_count})")
            self.quick_status.setStyleSheet("""
                padding: 4px 12px;
            ...
            self.quick_status.setText(f"⚠️ Partial connection ({active_count}/{total_count})")
            self.quick_status.setStyleSheet("""
                padding: 4px 12px;
            ...
            self.quick_status.setText(f"❌ No connections ({active_count}/{total_count})")
            self.quick_status.setStyleSheet("""
                padding: 4px 12px;
