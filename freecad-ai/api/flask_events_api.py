...
def trigger_event(event_type):
    ...
    if not handlers:
        return jsonify({'success': False, 'message': f'No handlers registered for event type: {event_type}'})
    try:
        for handler in handlers:
            handler.handle_event(event_type, data.get('data', {}))
        return jsonify({'success': True, 'message': f'Event {event_type} triggered successfully'})
