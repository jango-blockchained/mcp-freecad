    ...
    with _thread_pool_lock:
        if _thread_pool is None or _thread_pool._shutdown:
            _thread_pool = concurrent.futures.ThreadPoolExecutor(
                max_workers=2, thread_name_prefix="freecad_async"
            )
    return _thread_pool
...
def safe_create_task(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Failed to run task {task_name or 'unnamed'} in thread pool: {e}"
            )
            return False
    ...
    try:
        ...
        def run_async_in_thread():
            ...
            try:
                asyncio.run(coro)
                logger.debug(
                    f"Successfully completed task in thread pool: {task_name or 'unnamed'}"
                )
            except Exception as e:
                logger.error(
                    f"Error running task {task_name or 'unnamed'} in thread pool: {e}"
                )
        ...
        logger.error(
            f"Failed to submit task {task_name or 'unnamed'} to thread pool: {e}"
        )
        return False
...
def safe_emit_event(
    emit_func: Callable, event_type: str, event_data: Any, context: str = "unknown"
) -> bool:
    """
...
def freecad_safe_emit(
    emit_func: Callable, event_type: str, event_data: Any, signal_name: str = "unknown"
) -> None:
    """
    ...
    success = safe_emit_event(
        emit_func, event_type, event_data, f"freecad_signal_{signal_name}"
    )

    if not success:
        logger.warning(
            f"Failed to emit {event_type} event from FreeCAD signal {signal_name}"
        )
