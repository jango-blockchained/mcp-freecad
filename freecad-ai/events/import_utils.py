...
def safe_import_from_events(
    module_name: str, class_name: str, addon_dir: Optional[str] = None
) -> Optional[Type[Any]]:
    """
    ...
    try:
        ...
        if module_name == "base":
            ...
            return (
                EventProvider
                if class_name == "EventProvider"
                else getattr(
                    __import__(f".{module_name}", fromlist=[class_name], level=1),
                    class_name,
                )
            )
        else:
...
def safe_import_freecad_util(
    util_name: str, addon_dir: Optional[str] = None
) -> Optional[Any]:
    """
